...
[[maybe_unused]] static bool BeginComponentHeader(const char* label, bool& deleteRequested, bool defaultOpen = true)

{
    ...
    if (ImGui::Button("×", ImVec2(btnSize, btnSize))) deleteRequested = true;
...
[[maybe_unused]] static void EndComponentHeader() { ImGui::TreePop(); }



[[maybe_unused]] static void DragVec3(const char* label, Vector3& v, float speed = 0.05f)

{
    ...
    if (ImGui::DragFloat3("##v", buf, speed)) v = { buf[0], buf[1], buf[2] };
...
[[maybe_unused]] static void DragQuat(const char* label, Quat& q)

{
    ...
    if (ImGui::DragFloat4("##q", buf, 0.01f)) q = { buf[0], buf[1], buf[2], buf[3] };
...
[[maybe_unused]] static bool StrContainsCI(const std::string& haystack, const char* needle)
...
[[maybe_unused]] static void DrawTextureSlot(const char* slotLabel, Texture* tex, float thumbSize = 56.f)

{
    ...
    ImGui::SetCursorPosY(ImGui::GetCursorPosY() + (thumbSize - ImGui::GetTextLineHeight()) * 0.5f);
...
[[maybe_unused]] static void DrawTextureSlot(const char* slotLabel, unsigned int tex, float thumbSize = 56.f)
...
private:
    ...
    std::unordered_map<std::type_index, std::function<void(RenderPass*)>> passRenderMap;

    std::unordered_map<std::string, std::function<void(entt::registry&, entt::entity)>> componentAdderMap;

    std::unordered_map<std::type_index, std::function<void(entt::registry&, entt::entity, bool&)>> componentRenderMap;
